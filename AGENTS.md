...
- 当前只授权 C0 文档和环境检查。C0 完成后停止，等待用户批准 C1；文档中的计划、命令和默认值不构成下一阶段授权。
- 2026-09-12 用户另行批准创建 GitHub 仓库、Issues 等项目管理内容；本次允许初始化本地 Git、建立私有 `He1met/market-forecast-viewer`、上传文档并创建 Chart MVP 里程碑与 C0–C5 Issues。该授权不包含 C1–C5 实施、行情下载、公开仓库或网站部署。
...
- 无 Git/远程仓库不阻塞本地工作。最初 C0 不初始化 Git；本次建仓按上面的用户专项授权执行。后续按阶段建立可审查提交，其他推送、合并或可见性变更须有对应用户授权。数据提交范围与来源条款另行核实。
