...
import type{CanvasRenderingTarget2D}from'fancy-canvas';import type{Forecast}from'./contracts';import{utc}from'./chart';
...
export class ForecastBand implements ISeriesPrimitive<Time>{
 private update?:()=>void;drawCount=0;rendered:Vertex[]=[];
 ...
 constructor(private chart:IChartApi,private series:ISeriesApi<'Line'>,private forecast:Forecast){}
 attached(p:SeriesAttachedParameter<Time>){this.update=p.requestUpdate;this.update();}
 detached(){this.update=undefined;this.rendered=[];}
 paneViews(){return[this.view];}
 updateAllViews(){} // Coordinates are evaluated within the actual draw, after scale layout.
 private draw(target:CanvasRenderingTarget2D){const f=this.forecast;const data=[{time:f.anchor_time,inner_lower:f.anchor_price,inner_upper:f.anchor_price,outer_lower:f.anchor_price,outer_upper:f.anchor_price},...f.bands.points];const vertices:Vertex[]=[];
