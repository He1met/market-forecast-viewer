## Issue #44：07:53自然材料增量

业务窗口为05:53之后至07:53；08:17:29一致性捕获16个允许根的3256文件，72新增、5变化、3179引用已有core/audit/05:53增量链。包含06:18及07:19巡检原始收据、第二期预测完整输入输出及派生记录。104条公开内容/93对象、源覆盖与3180基础引用核验通过；首审确认字节与覆盖正确，修正根数文案17→16，并明确2711对象引用实例对应2706唯一SHA；保留v1，v2通过精确复审5753865521，08:28:45公开两附件4905777字节；完整远端下载SHA/大小、匿名三链接和原八资产不变核验通过。开发源22份单列实际较晚捕获时间，不冒充业务截止快照。

当前f5自然预测2/2分项通过，F0/F1仍1/20 waiting；其他验收保持开放。纯材料/文档工作未调用模型、补时段或切换生产包。精确附件、提交、独立审查、CI及远端校验随本批实际交付更新，见[07:53增量](MATERIALS_INCREMENT_20260921_0753.md)。

# 当前状态入口（2026-09-21）

## Issue #42：05:53自然证据增量

首次Issue40已独立验收关闭并由自然开发轮确认去重。新Issue42单独放行，保留首次六附件。以05:53为业务截止，在06:05:39用3.082秒短业务互斥取得前后一致快照；业务新增197/变化6、引用基础2,981条，另补明确相关开发与官方任务证据。04:29入口前停止没有业务收据，当前采集的官方任务与追加日志有独立时间标识；05:19完整成熟配对1/20、05:52自然预测1/2，不改其它验收结论。

复用已审导出工具；首轮因两处head/main加真实SHA的普通文字被编码检查拦住，保留失败，精确字面量独立核查通过后重做安全副本。最终236条映射、203对象、0blocked；源覆盖236/236及全部基础引用、六文件版本关系通过。精确附件独立审查5753082324通过后，两个附件于06:22:44公开；总计11,040,261字节全部实际下载SHA/大小一致，Release及下载链接匿名HTTP200。最终文档的精确审查/CI/合并以[PR #43](https://github.com/He1met/market-forecast-viewer/pull/43)回执为准。详情见[本批材料](MATERIALS_INCREMENT_20260921.md)。

最新安装与调度状态见[项目状态快照](PROJECT_STATUS.md)。PR #37已合并为main `27ee306`，正式包 `f5a0cc24`；切换前后完整备份/恢复及独立技术审查通过。03:49自然预测因任务提示仍固定旧包身份在入口前停止，未调用模型、未补slot；03:54监督侧修正三个业务任务提示，频率/模型不变，后续自然恢复仍待证。遗漏及修复见[部署补充](https://github.com/He1met/market-forecast-viewer/issues/39#issuecomment-5752269860)。

## Issue #40：项目材料公开（实施中）

2026-09-21 05:08补充：六附件已公开到[Release](https://github.com/He1met/market-forecast-viewer/releases/tag/materials-20260921-041032)，总计1,087,127,379字节。全部实际下载并核对SHA256/大小，与已审查的三核心卷、核心索引、审计卷及审计索引完全一致；原档未改写。核心31,528对象、审计45对象，各自清单保留完整逻辑路径。核心/审计独立审查分别为PR #41评论5752468886/5752485748，源码6e92c64独立审查5261705150和四项CI35535386002通过。最后仅更新下载链接、状态时点及未完Issue责任，最终文档提交仍按精确差异/CI审查后合并；本次不切换正式运行包。以下保留此前实施经过。

依据用户明确上传授权，新增公开副本工具、内容寻址映射、截图OCR辅助检查和分卷校验；原档不改写。短暂业务锁下获得正式快照，14份备份的86881逻辑文件逐项核对；私密对象不读，依赖/重复缓存和私密内容明确排除。图片包含687张PNG及196张嵌套JPEG，逐图范围核查与OCR结果分别记录，保留小字和OCR置信限制。

独立初查发现带前缀转义JSON、多字段Cookie和JSON键内认证信息的脱敏缺口，已修复并增加负例；首轮导出不发布，重新采集。独立源覆盖清点还发现采集后新增7个调度修正文件，已纳入新一轮；当前导出工作目录是明确排除的可变工作区，最终工具/测试证据另交付。随后固定03:39业务快照和04:10来源清单，69795个允许文件逐SHA/大小独立核对通过；04:18新包自然巡检10/10通过，作为单独补充，不冒充预测或方法效果成功。定向测试扩充至21项并通过；首个草稿head四项CI通过，最新修订仍须重新核验。附件上传、远端下载核验、最终精确提交审查和CI尚未完成，不把文档、清单或本地副本当作已公开交付。

请先查看 [当前交付与验收清单](CURRENT_ACCEPTANCE.md)。下文为按实际执行保存的历史记录，其中旧“未实现／未授权／暂停”只描述当时状态。当前主线已包含 PR #28/#29/#31，同类合并和切包遵循最新持续授权。

## Issue #39：严格核对已冻结但未调用的影子候选

正式预测两次失败后，影子候选仅留下冻结材料，巡检却读取不存在的 attempt。已保护双方原档、实验关系和来源共 48 文件，核对原运行包封存的 cycle／experiments／entry 控制流；不凭缺文件或单个 false 推断成功终止。

新增只读的独立证明读者：不可覆盖的证明绑定原字节、双方完整冻结输入、实验计划／登记／机会／时槽／观察、原包源码和精确实现审查，另逐次重验生产两次失败及候选无调用／发布／评分痕迹。只有全部成立才返回不可评分的“未执行”，不构造预测、不改变配对统计；预算／容量跳过、缺失、矛盾、部分调用与不支持的实现仍拒绝。后续无关运行、合法派生状态不会让历史证明失效。

新证明根纳入完整备份与恢复、兼容检查，并保全实验关联字节；不支持新证据根的旧包明确不可回退，维护失败须保留暂停及原备份，不能删除证明来恢复旧业务。原始 candidate／生产失败／generation 不改写，生产 proof 尚未写入。源码及合成测试不替代本次具体历史 statement、正式独立 review、精确候选与维护事务审查。

首次合成回归发现测试时钟未显式传入 cycle、派生索引 fixture 少 schema；并发排空测试依赖延迟计时而偶发只观测到7同时读取，改为八任务 rendezvous 后释放，不放宽并发上限／排空断言。原失败保留。文件备份不保留空 model-work 目录，证明仅规范化这两处无内容目录，全部文件和其它目录仍严格绑定。

## Issue #38：完整备份规模超时前置修复

PR #37 的原 head 两次远端完整备份门禁均在第二遍校验超出原 15 秒，失败证据保留；不再对同一提交重复尝试。合成完整规模剖析发现本机真实身份守卫占约一半耗时，单纯把读取增至 8 个仅约 3% 改善；将批内声明大小查询也有界并行后约 17% 改善，因此仅局部调整备份批次，不改变共享安全 helper 或缓存跳过校验。本机测量不代表 Linux CI 通过。

最多 8 个元数据查询或安全读取；全部元数据结束且声明总量不超过 256 MiB 后才读取内容，两阶段各自排空再处理失败。身份核验按下一批大小保证至多 32 文件，保留 250 毫秒调度与阶段边界；完整历史包、逐文件两次路径和字节校验、源增长拒绝、1 GiB／15 秒限制不变。新边界测试包括元数据失败／超时排空、不启动内容读取、32 文件守卫边界；精确测试、CI 与审查结果随后登记。
...
原运行、观察、收据及核验摘要先保留副本和字节清单；合成测试与真实原档只读回放不调用模型。Issue #34 备份实现保持。精确审查、CI、候选兼容和维护切换另记实际回执；原自然 2/2 只是历史证据，新失败及后续恢复独立记录。

## Issue #34：自然备份快照超时维护

自然备份于 2026-09-19 20:29 UTC 因 `BACKUP_SNAPSHOT_DEADLINE` 失败，未生成备份，原记录保留。完整扫描包含历史运行包，没有递归包含备份目标。此次维护保留全部备份目录、双读哈希、1 GiB 大小及 15 秒快照预算；将逐文件的昂贵进程身份核验改为最多 32 文件／250 毫秒检查边界与阶段末核验，文件路径与字节校验保持。慢系统调用返回后检查预算；此为协作式超时，不宣称硬实时中止。

成功清单和新的失败记录增加分阶段耗时与文件计数，原备份及原失败不改写。业务互斥覆盖完整双读捕获，释放后仅写已捕获内容；备份专有锁在清单与日槽提交前再次核验。合成规模验证覆盖 8500 个文件、超过 200 MB、九个运行包及全量恢复字节检查，并测试变化源、身份丢失、末次慢读取、大小和路径边界。首次 CI 规模测试在与全部 Node 测试并行时仍触发原 15 秒限制，失败保留；规模门禁改为同一质量流程中独立串行步骤，每次照常执行全规模和全恢复，另输出失败阶段诊断，预算不变。独立 CI 仍在第二遍读取时超时，进一步改为每批最多四个安全读取并行，批内文件声明大小合计不超过 256 MiB（大文件单独读取），失败时等待该批全部读取结束后才释放互斥，保留逐文件双读和预算校验。精确审查、CI、兼容证明与维护升级尚须后续回执；维护测试成功不代替下一实际自然备份或周日恢复验收。

## Issue #32：B 图表优先与运行验收收尾

首屏改为真实图表、最新／固定历史与六类概率。概率条按原始值排序，类别颜色固定，线宽/透明度按绝对概率映射，突出前三包含第三名并列，不改变概率。默认北京时间；各处显示时间随选择切换；预测依据、结果与学习和运行详情默认折叠。每分钟读取、页面返回恢复、固定历史、失败保留旧图及正式／影子候选边界保持。

运行摘要联合发布检查、巡检新鲜度和暂停状态；过去完成不冒充当前启用。页面解释任务内告警入口及送达未验证，不新增外部推送，也不把读取记为确认。统一任务矩阵区分工程、部署、用户视觉、自然备份复制、恢复自检、通知确认与方法效果；#12/#13及M1总体验收不自行关闭。

类型、单元、构建与实际浏览器验证随精确交付收据留存；最初类型检查发现索引尝试对象没有发布时间字段，已改用其真实创建时间。首轮浏览器18项通过、1项因测试仍查找旧按钮名失败，已随“返回最新”更新；实际窄屏截图发现控件裁切，补修并增加控件边界断言，失败原记录保留。此记录不宣称候选已经安装；精确审查、CI、合并、切包和自然验收进展见 Issue #32 最新回执。
