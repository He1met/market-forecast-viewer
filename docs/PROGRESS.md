## 2026-09-17 Issue #21：严格评分资格

修复ops及forecast fallback在资格判断前强读publication的问题。共用只读检查仅将两次耗尽且审计完整的未发布MODEL_ATTEMPT_REJECTED识别为不可评分；保留所有历史失败、独立评分游标、执行告警。独立审查首轮指出旧档缺event_audit及摘要可被篡改，返修已按精确历史解析器及调用参数重算原JSONL；真实旧失败run仅只读判定成功，20个原文件SHA不变，不改历史partial。缺档、损坏、成功后publication丢失、中断及不支持的失败类型继续unresolved。当前77a125自然ops的partial原结果不改写，正式包不替换。隔离正负例及现有质量检查通过后的精确提交另送独立审查。

## 2026-09-17 Issue #19：安装版配置提示

安装版状态接口依契约返回 configuration=null，页面改为“官方任务配置未接入本页；请在Codex任务中查看启用状态”，不再误称业务任务未配置。legacy未配置及已有配置回读保留原语义；本地暂停、真实尝试和下次官方时间未知保持不变。

验证：runtime浏览器专项1280-dpr1共5项通过（含installed-null、legacy-null、已有配置三个分支），10项相关运行状态契约检查与TypeScript检查通过；实际浏览器截图已查看。全部浏览器输入为隔离SYNTHETIC材料，没有行情或模型调用。此为开发分支修复，精确提交待独立审查；当前已授权运行77a125/build6423e13未替换。
...
监督发现旧tar附带719个AppleDouble元数据文件，已保留原档并另产仅677项payload（含manifest）的便携tar；新隔离目录解包后只读verifyPackage严格通过。首次本机Python不支持extractall的filter参数，未开始提取；改为显式逐项校验普通文件与相对路径后提取，失败记录保留，包校验未放宽。运维文档同步公开deploy的approval/config与HTTP检查、精确rollback参数/首次无previous、已实现备份观察和四类告警；交付表同步当前候选/CI/代码审查与安装预检边界。本次仅文档修订，需正常提交/CI、新候选身份及独立增量复核；不把旧SHA审查移植为新SHA批准。

业务PAUSED未变。真正安装/切包、受控服务与manual真实单轮须精确维护者确认；安装入口验档、旧档/备份恢复、官方任务实际配置与通知可见性仍待完成，自然两期和方法效果需未来实际证据。同盘备份仅local-recovery，首次实机previous rollback为NOT_AVAILABLE。

## 2026-09-16 附加输入计划与实验版本暂停

Issue15 FINAL-04：新增安装learning plan-input入口，支持calendar/derivatives单因素，绑定锁内精确有效策略哈希、当前包/配置；要求先有同生成身份的反馈20对决定和之后新成熟案例，同日限一次、同策略输入不重复。计划保存前置决定/案例revision/策略/发布包身份，doctor只读显示可核对的策略哈希。正式策略变化时预测登记和ops暂停实验，保留原档且正式预测继续；ops不自动覆盖暂停实验。旧晋级模型身份不匹配当前包时回到该包基础策略，损坏决定仍拒绝。

首审发现前置摘要未验档、手动入口可覆盖暂停状态、自动建计划未共享每日限额，已按三项P2修复。共享create在暂停或当日已有计划时拒绝，ops只在共享资格通过后创建；transition恢复不重建新计划。决定保存plan绑定、全机会快照和精确评分revision，独立commit marker绑定决定；附加输入前重新校验登记、冻结输入/策略、启动证据及原revision评分链，重算原先前20对决定，未知/禁用评分或撤销revision拒绝，不改历史结果。

最终quality Node232/232、Vitest177/177、typecheck/build及固定包三步通过。包内测试实际生成20对SYNTHETIC前向归档→成熟评分→反馈决定→新增验档案例→附加输入计划；覆盖缺机会、修改决定、计划错配、未知scorer、混合手动/自动每日限额、隔天暂停原字节保持。没有真实市场/模型调用或实际20对方法证据。修复测试初次缺合成根目录、模型收据字段及CLI版本格式不合契约，已修正fixtures并保留失败证据，未放宽生产校验。监督v2独立核对16文件SHA、两份completed及1675项内部证据，确认R1/R2/R3解决、FINAL-04 FIXED_VERIFIED / LOCAL_SLICE_PASSED；未重跑测试，随后仅补本段审查状态。业务PAUSED，完整工程CHANGES_REQUIRED，FINAL-01残余真实部署/运行验收仍未完成。

## 2026-09-16 服务巡检结果与独立告警

Issue15 FINAL-03：安装ops将服务检查移入业务锁内，ops暂停或互斥失败不重启服务。即使本小时结果核对已完成，仍检查服务；仅已确认退出的自有服务允许原有有界重启。健康、未知监听、身份冲突、版本不符、重启预算耗尽、启动失败及检查异常写入独立观察和最近巡检状态；服务异常使巡检partial，不能由评分成功掩盖。

服务告警独立于结果、执行和容量告警，普通故障连续两次触发、严重身份/存储故障立即触发，六小时提醒；暂停不伪报恢复，只有服务健康确认恢复。只读notifications汇总该流，pending不代表已送达。合成测试覆盖完成slot后故障、去重、暂停/恢复、检查异常、真实未知端口和自有服务重启预算；固定运行包覆盖ops→服务检查→pending摘要。首审发现服务故障会降级已完成的数据slot，已分开保存数据工作状态与本轮服务状态，新增重复故障/暂停/恢复后slot原字节及refreshInputs仅执行一次的回归。最终quality Node228/228、Vitest177/177、typecheck/build与固定包三步通过；监督复审核对9文件SHA、14份非文档受测副本、两份completed及757项内部证据，FINAL-03 FIXED_VERIFIED / LOCAL_SLICE_PASSED，未重跑测试。本段仅补审查状态，受测代码未改。正式安装entry、自然运行及通知呈现仍未验收；业务PAUSED，完整工程CHANGES_REQUIRED，下一步FINAL-04及FINAL-01残余验收。
...
Issue15 本片新增安装版notifications只读入口，汇总三类outbox、稳定事件集合ID、最多20条近期历史事件及省略数、记录的最后发布时间和核对动作。重复读取不改写outbox，未知/坏档不当作无告警，公开投影排除原始错误和路径。最后发布时间不冒充本轮验档；跨流快照不作事务一致性承诺。

官方文档及当前工具未给出逐条通知送达回执，当前无传输器，事件保持pending。运维文档准备官方业务任务呈现/去重规则，但尚未改变调度正文或证明实际通知可见。隔离quality Node199/199、Vitest177/177、typecheck/build和固定包三步通过，首次即通过；新增三流pending汇总、稳定身份、读取不改写、坏档拒绝与成功摘要降级回归。固定包验证真实模块组合，不绕过合成包激活拒绝；安装入口正式执行和通知呈现仍待验。监督独立核对8文件SHA、两套各6非文档副本、completed及全部内部证据，LOCAL_SLICE_PASSED；未重跑测试。本段仅更新审查状态，6受测文件不变；业务维护PAUSED，完整M1仍IN_PROGRESS。下一步本片验证归档后继续entry最终集成和维护者材料，正式通知可见性仍待验收。

## 2026-09-15 容量监测与可选工作限制

Issue15 本片增加运行包容量检查：默认安全余量2GiB、预测生成底线256MiB，可由安装配置capacity覆盖。安装版预测持业务锁后在领取slot前检查底线；候选准备前及正式发布后再次区分可选工作资格，低空间保留正式结果和capacity_skipped机会分母。备份入口检查源空间，目标检查取配置余量与容量策略余量较大值；开发验证在复制沙盒前检查2GiB余量。均不删除原档或自动清理证据。

ops持业务锁记录每日UTC基线和当前容量，明确仅为整个文件系统用量变化，不是项目归档增长；未知/异常统计不写为零，旧每日基线保留。低余量暂停候选结果扫描、新候选创建和附加数据刷新，正式结果核对仍可进行。容量告警使用独立pending流，避免巡检成功误报空间恢复；doctor返回即时容量。容量通过只代表检查时满足门槛，不保证后续磁盘写入成功。初轮隔离quality与package通过；独立初审发现低空间跳过候选时漏回传旧unresolved导致误恢复的P2，现修复为只读保留候选游标未解决项并标deferred，保持partial到真实成功重检。自查另修复候选准备前fresh容量检查、准备期容量跳过原因持久到最终结果；合批quality Node196/196、Vitest177/177、typecheck/build及固定包三步通过。监督v2独立核对11文件SHA、两套各9份非文档副本、completed与全部证据SHA，三项FIXED_VERIFIED、本片局部工程通过；未重跑测试。本段仅更新复审状态，9份受测代码/配置未改变。业务维护PAUSED，完整M1仍IN_PROGRESS，下一片官方通知与entry集成/最终验收材料。


## 2026-09-15 独立运行观察归集

Issue15 本片新增 ops 持业务锁后的独立 invocation 结果归集：冻结目录清单、每次最多读取16个结果，完成扫描后按完成时间消费持久队列。未知互斥/孤儿身份首次生成 critical outbox，普通执行失败连续两条后告警，预测同slot只计一次；暂停和重复slot不伪报恢复。execution告警与产出/巡检结果告警分开存储并合并pending摘要。中断后复用已持久的观察时间和ID，旧成功迟到不得恢复新故障。原始日志只读，未持锁者不竞争写共享状态。

隔离 quality Node188/188、Vitest177/177、typecheck/build通过；固定包实际未知端口占用→独立故障日志→合法持锁归集→pending去重链通过。无页面变更，不重跑浏览器。初次两份quality因owned ops故障重复进入执行流而失败；增加真实lock_acquired字段、保留owned ops原告警流后旧断言通过。独立审查另发现分批未完成仍标completed的P2；已将剩余扫描/消费纳入partial，新增33条结果真实ops同小时续接→全部消费→完成去重回归，并补同forecast slot普通故障去重。首败与初审均保留，监督v2独立复核8文件SHA、两套各7份非文档副本及全部证据SHA通过，初审P2为FIXED_VERIFIED，本片局部工程通过；未重跑测试。本段仅更新审查状态，7份受测代码/配置不变。只处理已落盘的result；只有started的中断轮保留并在下次重新查看，不推测失踪进程已结束。未知锁持续占用期间无法更新共享outbox，保留独立故障记录，待合法持锁巡检归集；即时外部通知仍待官方渠道集成。容量、entry和最终维护者材料继续待完成；业务维护PAUSED。

## 2026-09-15 安装包候选 cycle 编排验证

从干净 c8ff1d4 和匹配检查点续接 Issue15。新增固定包内真实 cycle 配合 experiment/archive/model 的合成编排验证：登记先于共同冻结、正式发布先于候选，覆盖候选成功、预算跳过和失败，核对最终机会记录、正式发布保留、同 slot 不重复调用及业务锁释放。官方输出、时钟和索引回调为合成替身，不声称 entry 配线、真实模型或及时性已验收。package 三步 exit0；监督独立核对两文件 SHA、completed 和515项内部证据，局部通过，未重跑测试。失败分支仅为候选执行前回调注入；重复 slot 仍读取策略，但不重复生成/最终归档。受测脚本未改；本段仅更新审查状态。待 GitHub 归档。

本轮未持锁告警归集仅完成代码核查，未修改；其顺序、持久去重和恢复语义继续待实施。容量、官方通知、entry 集成及最终维护者材料待完成。业务维护 PAUSED。

## 2026-09-15 安装包概率候选模块组合验证

从干净 909255f 和匹配检查点续接 Issue15。新增固定包内 experiment/archive/model 组合验证：共同市场与来源哈希、双输入绑定及正式冻结截止、未发布源拒绝、原始输出与路径保持、lambda=0.5 概率后处理、无模型进程执行记录和机会去重。package 三步通过；生产源码未改，无需重跑 quality/浏览器。初次测试清空 PATH 也屏蔽 ps，锁校验正确拒绝；修正隔离设置并保留首败。监督独立核对两文件 SHA、428项内部证据及三步 exit0，确认本片局部通过，未重跑测试；首次审查材料生成失败后已补齐并更正。待 GitHub 归档。

本片仅合成模块组合，不是完整 entry/cycle、真实模型、及时性或自然运行验收。容量、未持锁观察、官方通知、完整候选链和最终材料仍待完成。业务维护 PAUSED。

## 2026-09-15 自然续接：预测暂停周期与预期起点

从干净 `0fac964e485a538f73ce01fa2620ed0d7ca1d89b` 和匹配检查点续接 Issue15，fresh sync 63条来源。新增安装版 pause/resume 入口，由已批准固定包执行；仅改变预测意图，暂停清空起点，恢复建立新起点，重复恢复不重置。安装/回退配置从空起点开始。配置与起点单次原子提交，变更 intent 不冒充成功。预测在业务锁内重读暂停，ops 在锁内重读预期起点，避免启动快照过期。

合成隔离 quality 已通过 Node182/182、Vitest177/177、typecheck/build；覆盖暂停时段排除、幂等恢复、旧配置迁移、错误release、intent写入失败保留配置及锁内暂停阻止slot领取。固定安装包生命周期验证通过；监督已独立核对11文件SHA、两份completed与全部内部证据，确认本片局部工程通过，未重跑测试。quality副本早于新增verify-package测试与文档，package最终副本覆盖新增测试，其余受测代码一致；无页面变更，不重跑浏览器。业务维护 PAUSED，未真实恢复、切包或合并。完整M1仍 IN_PROGRESS；容量/未持锁观察/官方通知送达/共同冻结候选链及最终材料继续待完成。

## 2026-09-15 自然续接：预期时段缺产出核验

从干净 `454e6f7efacdc2a0edbcaad99be60d6a191a9ede` 和匹配检查点续接 Issue15，fresh sync 62条来源；未重复领取。安装配置显式 `forecast_expected_since` 划定连续启用段，暂停不累计过去时段，无起点为未知。只读状态核验最近两个已到期 slot 的生产原档，首未来节点加两分钟起显示停滞；每时段最多核验16个在合法生成窗口建立的run，超限或坏档返回未知。有效及时发布才满足预期，候选、旧锚点、晚发和任务成功摘要均不代替原档核验。

ops 在业务锁内对两个不同缺产出slot生成 warning outbox；单个slot被多次小时巡检不算两期失败，未知/暂停不制造恢复。页面明确这是本地计划推导，与实际调用状态分开。没有官方送达收据仍为 pending。

隔离验证：quality Node179/179、Vitest177/177、typecheck/build通过；固定安装包真实缺产出→outbox→同小时去重链与四视口/DPR浏览器4/4通过，已查看实际截图。quality 初次因旧版状态fixture剥离安装字段时遗漏新增字段失败，已修正fixture并保留首败。监督已独立核对15文件SHA、三组completed及内部证据哈希、生产文件快照一致性，并查看1280-dpr1截图；本片局部工程通过，监督未重跑测试。package/browser中的旧fixture仅少排除新增字段，最终fixture由quality覆盖。安装/暂停恢复自动维护预期起点仍是正式启用前必需待办，不宣称端到端暂停排除已验收。待GitHub归档。业务 PAUSED，真实单轮/自然运行/方法效果未验证。

## 2026-09-15 自然续接：安装版状态契约与巡检陈旧显示

本片从干净 `09e0717d183f1fd2c29c07f8960fe3dbd75de64f` 恢复 Issue15，fresh sync 61条来源、CHECKPOINT_MATCH；未重复领取。

- 修复安装版 `/api/m1/runtime` 返回已验证运行包、无官方配置回读时被页面契约拒绝的问题。明确 installed 来源，历史配置契约仍保留；安装版发布成功时间允许早于该流程结束，仍核对同轮/同预报和时间上下界。页面区分预报发布成功与整轮成功。
- 状态读取每次重新核对本地安装配置中的预测/巡检暂停意图，数据根改变拒绝读取；不将进程启动时配置永久当作当前状态，不伪称官方任务实时配置回读。
- 巡检新鲜度来自业务锁内提交的实际告警观察记录；90分钟内/超过90分钟/无观察/未来时钟异常分别显示。失败结果即使新鲜也保留异常，暂停不制造恢复，GET和备份不会刷新观察时间。只读接口不新增执行或告警。
- 隔离 quality：Node175/175、Vitest、typecheck/build通过；固定包内真实HTTP响应通过页面schema，暂停回读变化通过。新增状态面板浏览器检查4/4通过，覆盖四种视口/DPR，已查看实际截图。初次浏览器fixture误写一小时前的观察却标stale，被契约正确拒绝；修正为两小时前，并修正截图mkdir引用，原失败证据保留。生产逻辑未为测试放宽。

监督独立核对9文件SHA、各证据快照匹配范围和三份completed及内部证据SHA，通过本片局部工程审查；实际查看1280-dpr1面板，其他视口只核对通过证据，未重跑测试或代替用户视觉验收。本次仅更新文档审查状态，受测生产文件未改变。完整M1仍 IN_PROGRESS。缺产出、容量、未持业务锁故障归集、通知送达、共同冻结/概率候选安装链及最终维护者材料继续待完成。业务 PAUSED；安装版真实单轮、自然运行、方法效果仍未验证。
...
## 2026-09-17 Issue23：旧未发布预报的独立封存依据

自然巡检已证明Issue21原目标38850e04可正确排除评分；另一个早期ac8f0a3e只有一次已结束调用，原解析器把pre-turn chronicle提示拒绝。原始14文件、冻结源码SHA、JSONL和历史报告已只读核验；能证明调用结束，不能证明重试耗尽或实际调用参数。Issue23当前规划允许准备独立历史关闭依据，不改原收据。

新增严格封存读者，绑定完整原目录、历史/规划来源字节、当前独立封存review与被审实现SHA。生产ops与forecast fallback只消费完整通过的记录，保留失败展示和执行告警；兼容快照、备份与恢复回放包含封存来源。候选没有review时不可消费。测试仅SYNTHETIC，覆盖拒绝负例、无capture正常游标推进、幂等和恢复后复核。精确验证/提交见Issue23交付收据；当前c4420255业务继续，生产封存未写入、新包未切换，自然恢复仍未通过。

## 2026-09-17 Issue26：公开行情连接失败的有界同页重试

两条M1 history-candles采集路径共用窄GET函数。仅数字curl35且未取消/终止、有限deadline充足时同页额外一次；两次共用原单页35秒forecast/30秒outcome和整体预算，forecast重试保留90秒模型余量。每次请求独立保存不可覆盖started、响应及result；失败页不加入成功来源，原锚点/confirm/重复/分页/完整性校验保留。helper纳入固定包与冻结源码哈希，不改变模型重试、slot或TLS。

离线合成验证覆盖成功恢复、两次失败、取消/超时/证书/HTTP拒绝、预算耗尽与模型余量、第五页不重抓前四页、跨锚点拒绝、原数据负例、准备失败不冻结/不调用模型、旧评分保留及新attempt备份恢复。质量与运行包检查已通过，最终精确收据随交付报告绑定。第一次新增恢复测试误断言了不存在的receipt.status，已按实际replay.passed与activation_restored字段修正，原失败日志保留；未放宽字节恢复断言。

本次仅工程修复；正式d7d033包保持运行，没有补跑行情/模型/旧slot。连接重试能否改善真实自然运行尚待新包独立审查和部署后证据。Issue27未冻结准备失败分类单独规划，不在本次代码范围。

## 2026-09-17 Issue #27（实施中，尚未部署）

- 已实现严格的冻结前准备失败收据、观察记录绑定、评分跳过及缺报计数；原失败状态保持不变。
- 已实现原封存四文件不变的增量实现审查，以及独立的旧准备失败证明读取；新增证据根纳入备份、兼容性与恢复，禁止无支持旧包回退。
- 离线定向测试、完整质量检查（177 项 Vitest、290 项 Node、类型及构建）和打包检查通过；保留首轮旧断言失败及修复记录。另补充新证明根备份恢复和陈旧实现拒绝测试后，提交前再次核验最终文件。精确提交独立审查、候选包与真实旧记录证明验证仍待完成。
- 当前运行包仍为 Issue #26 的发布；本次代码和合成测试未修改正式原档、运行配置或启用新的业务调用。

- PR #29 初次独立审查发现：结构化失败收据与最终观察错误原因、task/thread 身份未严格比较。已补一致性门禁及三类矛盾负例，旧 head 审查不通过；修正后须重新核验测试、精确 head 及证明实现映射。

## 2026-09-19 Issue #30：CLI 启动提示版本适配（工程验证中）

恢复后一次自然预测的两次尝试均正常结束，但 CLI 0.155.0-alpha.9 的精确已禁用 Code Mode 启动提示被旧版本适配器拒绝，整期未发布。已核对实际参数、完整事件顺序及原始审计，20个原文件保持不变。

当前适配只支持两个实测精确 CLI 版本和完整安装调用参数；未知版本、额外参数、错误位置、重复提示、未知错误和工具事件仍拒绝。历史 b280 解析器的审计语义单独保留，按冻结代码身份重放原收据；此次旧失败仍显示失败，只在两次终止及审计完整一致时排除评分，不补发布或重跑旧时段。首次新增测试因测试变量遮蔽失败，修正后质量检查通过，原失败日志保留。

精确提交审查、CI、候选包兼容与备份恢复、原封存及准备失败证明的新增实现审查、正式升级和后续自然恢复另行留证；当前工程测试不代表已经上线。
