...
修复ops及forecast fallback在资格判断前强读publication的问题。共用只读检查仅将两次耗尽且审计完整的未发布MODEL_ATTEMPT_REJECTED识别为不可评分；保留所有历史失败、独立评分游标、执行告警。缺档、损坏、成功后publication丢失、中断及不支持的失败类型继续unresolved。当前77a125自然ops的partial原结果不改写，正式包不替换。隔离正负例及现有质量检查通过后的精确提交另送独立审查。
