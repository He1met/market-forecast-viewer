...
复用已审导出工具；首轮因两处head/main加真实SHA的普通文字被编码检查拦住，保留失败，精确字面量独立核查通过后重做安全副本。当前236条映射、203对象、0blocked；源覆盖236/236及全部基础引用、六文件版本关系通过。精确附件独立审查、真实上传/下载及本次文档PR交付待后续回执，不将本地完成当已公开。详情见[本批材料](MATERIALS_INCREMENT_20260921.md)。
