...
成功清单和新的失败记录增加分阶段耗时与文件计数，原备份及原失败不改写。业务互斥覆盖完整双读捕获，释放后仅写已捕获内容；备份专有锁在清单与日槽提交前再次核验。合成规模验证覆盖 8500 个文件、超过 200 MB、九个运行包及全量恢复字节检查，并测试变化源、身份丢失、末次慢读取、大小和路径边界。精确审查、CI、兼容证明与维护升级尚须后续回执；维护测试成功不代替下一实际自然备份或周日恢复验收。
