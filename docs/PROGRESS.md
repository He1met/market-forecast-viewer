...
当前状态：M0/C1–C5、#9工程与用户五项视觉验收通过；#11及#12均已通过正式工程审查并自然轮去重确认，#12新增实验页面的用户体验验收仍待确认。#13 v1 已完成真实结果核对、独立归档及图表叠加，首个真实6h窗口核对成功，相关验证通过，待PR #7规划侧审查。12h/24h仍按真实成熟状态保留部分观测，24h Brier尚未评分；尚无single_run_verified，#14及两小时业务任务未启用。当前批准阶段M1，M2及以后仍需用户新的明确授权。feat/chart-mvp，原始行情、模型输入输出、截图与完整证据保持LOCAL_ONLY。
