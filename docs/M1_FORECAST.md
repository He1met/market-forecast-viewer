...
对应 #10 v2 / #11 v1。M0 已交付且五项视觉验收通过；M1 已获批准。本阶段先实现本地单次 `prepare → Codex → validate/publish`，#12 后续接图表，#13 后续核对真实到期结果。#14 的两小时业务任务仍需 `single_run_verified`。工程能运行不等于预测准确、校准或盈利。
...
整个run及原始响应均gitignored、LOCAL_ONLY、位于网页可服务目录之外。公开PR只提供非价格run_id、真实时间、SHA及测试摘要。没有新增网页入口、网格盈亏、交易、业务定时器或公网部署。
