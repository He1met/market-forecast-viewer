...
2026-09-16流程更新：用户直接授权“合并，项目以后可以自行合并”，替代本文件历史不合并限制。当前按AGENTS的已批准范围、必要测试、独立审查及精确head门禁正常squash合并；不改变仓库可见性，不自动激活正式包或恢复业务。

状态：M0/C1–C5及#9说明修正已工程交付，用户五项视觉验收已通过；#9自然轮审查确认已完成。本文件保留M0图表设计基线，历史“不做预测”等条款描述该基线，不限制用户后续批准的开发阶段。当前实施/领取证据以PROGRESS与executor检查点为准。

2026-09-13最新流程授权：现有chart-mvp执行本项目所有规划侧明确放行且依赖满足的开发任务，不按M0/M1或Issue编号限制。每轮持锁同步、核验真实来源/版本/正文/依赖后领取，相关测试后自动提交推送与报告；正式规则见AGENTS。M1已有用户阶段批准，旧DEMO三文件保留，实验输入输出分开存储；业务定时启用仍遵循具体任务条件。
