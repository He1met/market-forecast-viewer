...
状态：C0 约定已形成，待用户批准 C1。2026-09-12。以下功能均为计划，尚无页面实现。
...
无真实预测、收益/胜率/推荐等级、回测、撮合、自动交易、定时任务、自动更新、数据库、Docker、登录或公网部署。本轮不接入其他大模型、付费大模型 API。来源下载失败明确报告，不能把合成价格写进历史文件。
...
| --- | --- | --- |
| C0 约定 | 开发包/本机环境 → 五份最小文档 | 目录/Git/版本/官方接口检查与字段方案可审查；停止等批准 |
...
| C3 路径与区间 | 固定三文件/C2 → `src/forecast-band.ts`、路径/分界/图例与相关测试 | 三条路径、内外带、阶段、全隐藏/纯未来/resize/DPR/价轴验证；第一轮用户视觉验收后再推进 |
...
每阶段记录真实命令与结果；授权以用户消息为准，C0计划不自动授权 C1–C5。2026-09-12 用户专项批准建立 GitHub 私有仓库、上传文档和创建任务；使用 `He1met/market-forecast-viewer` 与 Chart MVP 里程碑管理 C0–C5。Issue 创建或依赖完成不授予实施权限；本次之后不自动推送、合并或改变可见性。
