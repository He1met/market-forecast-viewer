...
## 2026-09-16 受控运行失败与兼容返修

236180d9/build612e2496经用户精确批准已安装；备份及隔离恢复完成，三个官方业务任务PAUSED。合法13:47 UTC的manual真实冻结成功，但两次脚本内模型尝试因同一CLI启动诊断被审计拒绝，未发布，single_run未通过。三项运行暂停均恢复，自有服务停止。CLI-STARTUP-NOTICE-01只修精确禁用工具上下文中的前置notice分类，不启用host、不宽泛放过错误、不修改原失败收据或复用该slot；下一实机验证须核对新精确包和未来合法slot授权。下面各历史安装/失败记录保留其时点含义。
