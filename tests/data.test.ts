...
describe('真实本地快照与固定演示',()=>{
 ...
 it('等比计算溢出不能绕过分布校验',async()=>{const x=structuredClone(g);Object.assign(x.schemes[0],{lower_price:1e-300,upper_price:1e300,interval_count:2,distribution:'geometric',levels:[1e-300,123,1e300]});await expect(validateGrids(await seal(x),h,f)).rejects.toThrow('网格分布计算溢出');});
