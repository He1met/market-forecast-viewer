import {verifyProtocolEvidence,verifyConfigBoundary,verifyExistingCoordinates,installedLauncher} from './m1-install-protocol.mjs';
import {verifyTargetArchives,archiveSnapshot} from './m1-compatibility.mjs';
import {commitInstallation,readInstallationTransaction,recoverInstallationFiles} from './m1-install-commit.mjs';
...
export async function activate({runtimeHome,releaseId,approval,config,expectedCurrent=null,healthCheck=false}){
 await assertInstallationSettled(runtimeHome);
 await verifyExistingCoordinates(runtimeHome,config);
 ...
 verifyProtocolEvidence({manifest,approval,launcher:await readBytes(runtimeHome,path.join(runtimeHome,'releases',releaseId,'runtime/launch.mjs'))});
 ...
 if(previous){const oldManifest=await verifyPackage(path.join(runtimeHome,'releases',previous.release_id)),oldApproval=await readJson(runtimeHome,path.join(runtimeHome,'approvals',previous.release_id+'.json'));check(oldManifest.release_id===previous.release_id&&oldManifest.build_sha===previous.build_sha,'PREVIOUS_PACKAGE_CHANGED');verifyProtocolEvidence({manifest:oldManifest,approval:oldApproval,launcher:await installedLauncher(runtimeHome)});const installed=await validateInstallation(path.join(runtimeHome,'installation.local.json'));verifyConfigBoundary(installed,config);check(installed.forecast_paused===true&&installed.ops_paused===true&&installed.service_paused===true,'PAUSE_BEFORE_ACTIVATION');}
 ...
 const health=healthCheck?await verifyDeploymentHealth({packageRoot:path.join(runtimeHome,'releases',releaseId),dataRoot:config.data_root,releaseId,port:config.http_port}):null;
 ...
 if(health)await writeOnce(runtimeHome,path.join(runtimeHome,'health-checks',randomUUID()+'.json'),health);
 ...
 const committed=await commitInstallation({runtimeHome,guard:mutex.guard,rollbackGuard:async()=>{
  if(previous){const oldRoot=path.join(runtimeHome,'releases',previous.release_id),oldManifest=await verifyPackage(oldRoot);check(oldManifest.release_id===previous.release_id&&oldManifest.build_sha===previous.build_sha,'PREVIOUS_PACKAGE_CHANGED');const oldApproval=await readJson(runtimeHome,path.join(runtimeHome,'approvals',previous.release_id+'.json'));verifyProtocolEvidence({manifest:oldManifest,approval:oldApproval,launcher:await readBytes(oldRoot,path.join(oldRoot,'runtime/launch.mjs'))});await verifyTargetArchives({packageRoot:oldRoot,dataRoot:config.data_root,releaseId:previous.release_id});}
 },files:{
  'launch.mjs':await readBytes(runtimeHome,path.join(runtimeHome,'releases',releaseId,'runtime/launch.mjs')),
  'installation.local.json':{...config,forecast_expected_since:null},
  'current.json':{schema:'MFV:CURRENT:v1',release_id:releaseId,build_sha:manifest.build_sha,previous_release_id:previous?.release_id??null,activated_at:new Date().toISOString()}
 }});
 return{status:'activated_paused',release_id:releaseId,health,installation_commit:committed};
...
export async function rollback({runtimeHome,releaseId}){await assertInstallationSettled(runtimeHome);const current=await readJson(runtimeHome,path.join(runtimeHome,'current.json'));check(current.previous_release_id,'NO_PREVIOUS_RELEASE');check(/^[a-f0-9]{64}$/.test(releaseId??'')&&releaseId===current.previous_release_id,'EXACT_PREVIOUS_RELEASE_REQUIRED');const approval=await readJson(runtimeHome,path.join(runtimeHome,'approvals',current.previous_release_id+'.json')),config=await readJson(runtimeHome,path.join(runtimeHome,'installation.local.json'));return activate({runtimeHome,releaseId,approval,expectedCurrent:current,config:{...config,forecast_paused:true,ops_paused:true,service_paused:true}});}
// Recovery obtains mutex coordinates from the hash-bound intent, never mixed live files.
export async function recoverInstallation({runtimeHome,transactionId}) {
 const tx=await readInstallationTransaction(runtimeHome,transactionId),configEntry=tx.entries.find(x=>x.name==='installation.local.json');
 const config=JSON.parse((configEntry.before??configEntry.after).toString()),afterConfig=JSON.parse(configEntry.after.toString());verifyConfigBoundary(config,afterConfig);
 const checkFile=path.join(runtimeHome,'.recovery-check-'+randomUUID()+'.json');
 await writeOnce(runtimeHome,checkFile,config);try{await validateInstallation(checkFile);}finally{await fs.unlink(checkFile);}
 check(config.runtime_home===runtimeHome&&config.forecast_paused===true&&config.ops_paused===true&&config.service_paused===true,'RECOVERY_REQUIRES_PAUSED_CONFIG');
 const mutex=await businessMutex({dataRoot:config.data_root,port:config.mutex_port,releaseId:'installation-recovery',task:'admin'});check(mutex.status==='ACQUIRED',mutex.status);
 try {
  check(['not_started','exited'].includes((await serviceStatus({runtimeHome,port:config.http_port})).status),'STOP_OWNED_SERVICE_BEFORE_RECOVERY');
  return await recoverInstallationFiles({runtimeHome,id:transactionId,guard:mutex.guard,verifyTarget:async({transaction,side})=>{
   const item=transaction.entries.find(x=>x.name==='current.json'),current=JSON.parse((item[side]??item.after).toString());
   check(/^[a-f0-9]{64}$/.test(current.release_id??''),'CURRENT_RELEASE_INVALID');
   const packageRoot=path.join(runtimeHome,'releases',current.release_id),manifest=await verifyPackage(packageRoot),approval=await readJson(runtimeHome,path.join(runtimeHome,'approvals',current.release_id+'.json'));
   check(manifest.synthetic!==true&&manifest.release_id===current.release_id&&manifest.build_sha===current.build_sha&&approval.schema==='MFV:MAINTAINER_APPROVAL:v1'&&approval.approved===true&&approval.release_id===current.release_id&&approval.build_sha===current.build_sha&&/^https:\/\/github\.com\/He1met\/market-forecast-viewer\/(?:pull|issues)\//.test(approval.source_url)&&Number.isFinite(Date.parse(approval.approved_at)),'RECOVERY_EXACT_APPROVAL_REQUIRED');
   const launcherEntry=transaction.entries.find(x=>x.name==='launch.mjs');verifyProtocolEvidence({manifest,approval,launcher:launcherEntry[side]??launcherEntry.after});
   await verifyTargetArchives({packageRoot,dataRoot:config.data_root,releaseId:current.release_id});await mutex.guard();
  }});
 }finally{await mutex.close();}
}
