...
try {
 ...
 let list=commands[mode];if(!list){const names=(await fs.readdir(path.join(work,'tests'))).filter(x=>x.endsWith('.test.mjs')).sort().map(x=>'tests/'+x);list=[['node_modules/typescript/bin/tsc','--noEmit'],['node_modules/vitest/vitest.mjs','run',...(await fs.readdir(path.join(work,'tests'))).filter(x=>x.endsWith('.test.ts')).map(x=>'tests/'+x)],['--import','tsx','--test',...names],['node_modules/vite/bin/vite.js','build']];if(mode==='all')list.push(...commands.browser);}
