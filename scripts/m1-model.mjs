...
import{readFrozen,assertFrozenCode,prepareAttempt,completeAttempt,publishRun,readPublished}from'./m1-archive.mjs';import{auditCodexEvents}from'./m1-forecast.mjs';import{managedProcess}from'./m1-mutex.mjs';import{writeOnce,check}from'./m1-files.mjs';
...
export async function generateInstalled(input,{mutex,timeoutMs,beforePublish}){
 ...
 let result;try{result=await managedProcess('codex',args,{mutex,cwd:workspace,env,stdoutFile,stderrFile,timeoutMs:remaining,input:frozen.prompt});}catch(e){result={code:-1,timedOut:false,error:e.message};}
 const stream=await fs.readFile(stdoutFile,'utf8').catch(()=>''),audit=auditCodexEvents(stream),raw=await fs.stat(attempt.rawFile).catch(()=>null),valid=result.code===0&&!result.timedOut&&raw?.isFile()&&audit.turn_completed&&!audit.failed&&audit.unexpected_count===0;
 await completeAttempt(runDir,attempt.attempt_id,{exit_code:valid?0:-1,cli_exit_code:result.code,timed_out:result.timedOut,error:valid?null:result.error??'MODEL_ATTEMPT_REJECTED',model_config:{provider:'official_codex',selection:'existing_local_cli_configuration',cli_version:cliVersion,auth_method:'chatgpt_verified',sandbox:'read-only',output_schema:true,startup_warning_count:audit.startup_warning_count},model_identity:null,model_identity_visibility:'not_exposed_by_jsonl',model_thread_id:audit.events.find(x=>x.type==='thread.started')?.thread_id??null,unexpected_tool_events:audit.unexpected_count});
