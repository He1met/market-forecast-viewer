...
export async function candidateEvidence(dataRoot,id){
 ...
 const last=await readJson(dataRoot,path.join(officialDirectory,'attempt-002/receipt.json'));
 const times=[start.started_at,slot.claimed_at,registered.registered_at,official.manifest.information_frozen_at,candidate.manifest.information_frozen_at,frozen.frozen_at,last.ended_at,generation.completed_at,end.completed_at];check(times.every(iso)&&times.every((x,i)=>!i||Date.parse(x)>=Date.parse(times[i-1]))&&Date.parse(end.completed_at)<=Date.now(),'CANDIDATE_TIME_ORDER');
