...
export function experimentStore(dataRoot,{afterTransitionWrite=async()=>{}}={}){
 ...
 async function review({readMetrics,guard}){const recovered=await recoverTransition({guard});if(recovered)return recovered;const plan=await active();if(!plan)return null;const directory=path.join(home,plan.id),decisionFile=path.join(directory,'decision.json');const savedDecision=await exists(decisionFile)?await readJson(dataRoot,decisionFile):null;const entries=savedDecision?[]:await fs.readdir(path.join(directory,'opportunities')).catch(e=>{if(e.code==='ENOENT')return[];throw e;}),opportunities=[];
  for(const id of entries){const dir=path.join(directory,'opportunities',id),r=await readJson(dataRoot,path.join(dir,'registered.json')),g=await exists(path.join(dir,'generation.json'))?await readJson(dataRoot,path.join(dir,'generation.json')):null,f=await exists(path.join(dir,'frozen.json'))?await readJson(dataRoot,path.join(dir,'frozen.json')):null;const start=await candidateStart(f);const control=f?await readMetrics(f.official_run_id,'production'):null,candidate=f?await readMetrics(f.candidate_run_id,'candidate'):null;opportunities.push({...r,started:!!start,candidate_attempt_started_at:start?.started_at??null,generation_status:g?.candidate_status??'interrupted_or_pending',finished:Date.now()/1000>=r.anchor_time+86400,complete:!!start&&control?.complete===true&&candidate?.complete===true,control,candidate});}
  const result=savedDecision??evaluateExperiment(plan,opportunities);if(result.decision==='waiting')return result;await guard();
  const decision=savedDecision??{...result,experiment_id:plan.id,decided_at:new Date().toISOString(),effective_after:new Date().toISOString(),production_config:result.decision==='promote'?plan.candidate:plan.control};
 ...
 return{active,create,register,freezeCandidate,finish,review,recoverTransition};
}
