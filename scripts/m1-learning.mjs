...
export function feedback(cases,features,cutoff){
 ...
 for(const k of keys){const a=past.map(c=>c.features[k]).filter(Number.isFinite);if(a.length<1)continue;const m=mean(a),sd=Math.sqrt(mean(a.map(x=>(x-m)**2)));scales[k]=sd>0?sd:1;}
 ...
 const chosen=[];let body='';for(const {c,distance}of ranked){if(chosen.length===4)break;const item={id:c.id,revision:c.revision_id,distance,actual_category:c.actual_category,main_scenario:c.main_scenario,path_loss:c.path_loss,brier:c.brier,diagnostics:c.diagnostics??[]};const next=JSON.stringify({builder:'numeric-similarity-v1',eligible_history_count:past.length,cases:[...chosen,item]});if(Buffer.byteLength(next)>8192)continue;chosen.push(item);body=next;}
 body ||= JSON.stringify({builder:'numeric-similarity-v1',eligible_history_count:past.length,cases:[]});return{schema:'MFV:FEEDBACK:v1',cutoff,case_ids:chosen.map(x=>x.id),revisions:chosen.map(x=>x.revision),body,sha256:digest(body),bytes:Buffer.byteLength(body)};
}
